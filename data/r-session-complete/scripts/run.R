...
jsondata <- fromJSON(
  file = "https://raw.githubusercontent.com/rstudio/rstudio/main/src/cpp/session/resources/dependencies/r-packages.json"
)
pnames <- c()
for (feature in jsondata$features) {
  pnames <- unique(c(pnames, feature$packages))
}
...
getbiocreleasedate <- function(biocvers) {
  ...
  if (identical(repodate, character(0))) {
    repodate <- "latest"
  }
...
getreleasedate <- function(repodate) {
  ...
  while (!URLfound) {
    if (
      !RCurl::url.exists(
        paste0(repo, "/src/contrib/PACKAGES"),
        useragent = "curl/7.39.0 Rcurl/1.95.4.5"
      )
    ) {
...
paste(
  "Defining repos and setting them up in repos.conf as well as Rprofile.site"
)
...
cat(
  '  Sys.setenv(R_LIBS_USER = paste0(Sys.getenv("R_LIBS_USER_BASE_PATH"), substring(Sys.getenv("R_LIBS_USER"), 2)))\n'
)
cat(
